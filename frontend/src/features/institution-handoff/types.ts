...
export type EvidenceBundleItem = { id: string; category: 'situation' | 'fact' | 'profile' | 'action' | 'question' | 'document' | 'warning'; title: string; description: string; literal?: string; included: boolean; required: boolean; source: 'user' | 'confirmed_facts' | 'action_guidance' | 'demo_profile' }
