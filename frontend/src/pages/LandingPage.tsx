  ...
  return <main className="landing-shell" style={{ backgroundImage: `url(${heroImage})` }}>
    ...
    <section className="hero-section" id="main-content" lang={language}>
      <div className="hero-row">
        ...
        <aside className="hero-steps" aria-label={content.landing.stepsLabel}>
          <ol>
            {content.landing.steps.map((step) => <li key={step.number}>
              <span className="hero-step-number" aria-hidden="true">{step.number}</span>
              <span className="hero-step-text"><strong>{step.title}</strong><span>{step.description}</span></span>
            </li>)}
          </ol>
        </aside>
      </div>
