    ...
    <section className="guidance-layout" aria-live="polite">
      ...
      {status === 'complete' && result && <>
        ...
        <section className="obligation-list">{visible.length === 0 ? <p className="empty-state">{copy.empty}</p> : visible.map((item) => {
          ...
          const reviewRequired = evidence?.verificationStatus === 'review_required'
          const deadlineNeedsReview = reviewRequired || item.deadline === null
          return <article className={`obligation-card ${reviewRequired ? 'evidence-review' : `urgency-${item.urgency}`}${done ? ' done' : ''}`} key={item.id}><div className="obligation-top"><span className="party-label">{copy.parties[item.party]}</span><span className="urgency-label">{deadlineNeedsReview ? copy.reviewDeadlineStatus : <>{copy.urgency[item.urgency]}{item.daysRemaining !== null ? ` · ${copy.daysRemaining(item.daysRemaining)}` : ''}</>}</span></div><h2>{copy.obligationText[item.title]}</h2><p>{copy.obligationText[item.description]}</p><div className="deadline-row"><strong>{deadlineNeedsReview ? copy.reviewDeadlineLabel : copy.deadline}</strong><span>{formatDate(item.deadline, localeMap[language]) ?? copy.dateNeeded}</span></div><div className="document-list"><strong>{copy.documents}</strong>{item.requiredDocuments.length ? <ul>{item.requiredDocuments.map((doc) => <li key={doc.id}><FileText size={14} aria-hidden="true" />{copy.documentText[doc.name]} <small>{copy.documentText[doc.note]}</small></li>)}</ul> : <span>{reviewRequired ? copy.documentsNeedReview : copy.noDocuments}</span>}</div>
            {evidence && (reviewRequired ? <p className="evidence-pending">{copy.sourcePending}</p> : <details className="evidence-details"><summary>{copy.evidence}</summary><dl>{(['sourceTitle', 'issuingAgency', 'effectiveFrom', 'checkedAt', 'verificationStatus'] as const).map((key) => <div key={key}><dt>{copy.evidenceLabels[key]}</dt><dd>{key === 'sourceTitle' ? evidence.sourceTitle : key === 'issuingAgency' ? evidence.issuingAgency : key === 'verificationStatus' ? copy.verification[evidence.verificationStatus] : evidence[key] ?? copy.sourcePending}</dd></div>)}</dl>{evidence.sourceUrl && <a href={evidence.sourceUrl} target="_blank" rel="noreferrer">{evidence.sourceTitle}</a>}</details>)}
